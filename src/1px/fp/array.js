/// Array
import {_, filterCallback, mapCallback} from "./fp.js";
...
_.array_difference = (a, b, callback = _.itself) => a.filter(x => !b.map(callback).includes(callback(x))).concat(b.filter(x => !a.map(callback).includes(callback(x))));
