import {_} from "../fp"
import {filterCallback, mapCallback} from "../fp/fp.js";
...
export const bind = (onStart, onNext, onError = noop, onComplete = noop) => {
  if (!_.isFunction(onStart)) onStart = noop;
...
export const trace = (...tag) => lift((observer, prev) => ({
  next(value) {
    console.log(...tag.map(tag => "\x1b[34m" + tag), ":", prev, "→", value);
    observer.next(value);
    prev = value;
  },
...
export const throttleTime = (duration) => throttle(() => Observable.timeout(duration));

export const withLatestFrom = (...other) => (observable) => {
  ...
  return lift(observer => ({
    start() {
      ...
      Observable.combineLatest(...other).subscribe({
        next(value) {
          emitted = true;
          value2 = value;
